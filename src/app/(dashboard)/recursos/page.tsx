...
async function getRecursos() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      `
      tipo, costo_total, descripcion, fecha, proveedor,
...
export default async function RecursosPage() {
  ...
  for (const c of costos) {
    if (!porTipo[c.tipo]) porTipo[c.tipo] = { total: 0, cantidad: 0, items: [] }
    porTipo[c.tipo].total += c.costo_total ?? 0
    porTipo[c.tipo].cantidad += 1
    porTipo[c.tipo].items.push(c)
  }

  const totalGeneral = costos.reduce((s, c) => s + (c.costo_total ?? 0), 0)

  // Últimos 10 registros para tabla reciente
  ...
  return (
    <div>
      <Header
        titulo="Recursos"
        subtitulo="Resumen de costos por tipo de recurso"
      />
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-gradient-to-r from-slate-900 to-slate-700 rounded-2xl p-6 text-white">
          ...
          <p className="text-sm text-slate-400 mt-1">
            {costos.length} registros de costo
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {Object.entries(TIPO_CONFIG).map(([tipo, cfg]) => {
            const data = porTipo[tipo] ?? { total: 0, cantidad: 0 }
                ...
                <CardContent className="pt-4 pb-4">
                  ...
                  <div className="mt-2">
                    <div className="h-1.5 rounded-full bg-slate-100">
                      <div
                        className={cn("h-1.5 rounded-full", cfg.bg, cfg.color, "bg-current opacity-50")}
                        style={{ width: `${Math.min(pct, 100)}%` }}
                      />
                    </div>
                    <p className="text-xs text-slate-400 mt-1">
                      {pct.toFixed(0)}% del total · {data.cantidad} registros
                    </p>
                  </div>
                </CardContent>
              </Card>
          ...
          {recientes.length === 0 ? (
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {recientes.map((c, i) => {
                    const cfg = TIPO_CONFIG[c.tipo] ?? TIPO_CONFIG.indirecto
                    const Icon = cfg.icon
                    return (
                      <tr
                        key={i}
