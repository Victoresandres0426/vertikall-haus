...
Notas importantes:
- MUY IMPORTANTE — "dia_inicio" y "dia_fin": si el archivo ya trae columnas explícitas con estos valores (aunque tengan otro nombre: "inicio"/"fin", "start"/"end", "día X", etc.), COPIA esos números EXACTOS tal cual aparecen — no los redondees, no los recalcules, no los reemplaces por una acumulación de duraciones. Esos valores fueron calculados por el estimador y reflejan cuadrillas trabajando en paralelo (varias actividades pueden compartir o solaparse en los mismos días porque las hace gente distinta) — es normal y correcto que actividades de distintas filas se traslapen en el tiempo, NO fuerces que cada actividad empiece justo cuando termina la anterior. Solo estima acumulando duraciones en el orden en que aparecen como último recurso, si el archivo de verdad no trae ningún dato de fecha/día por actividad.
