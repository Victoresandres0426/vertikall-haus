-- ============================================================
-- 080 — Fix: "column reference proyecto_id is ambiguous"
-- ============================================================
-- generar_facturas_semanales() RETURNS TABLE(proyecto_id UUID, ...),
-- es decir "proyecto_id" es a la vez un parámetro de salida de la
-- función y el nombre de una columna en varias tablas que la función
-- consulta (actividades, facturas_cliente, presupuestos, los
-- snapshots...). Las comparaciones que dejaban "proyecto_id" sin
-- calificar (sin alias de tabla) quedaron ambiguas para Postgres en
-- cuanto la función se ejecutó de verdad con la lógica nueva de la
-- 078/079 (antes nunca se había corrido esta versión completa).
--
-- Este fix reescribe la función completa calificando con alias de
-- tabla TODAS las columnas en TODAS las consultas, para eliminar la
-- ambigüedad de raíz sin depender de cómo Postgres decida resolverla
-- caso por caso. No cambia ninguna lógica de negocio.
-- ============================================================
