-- ============================================================
-- 085 — Agrupar indirectos en una línea, mover indirectos/utilidad
--        al final sin mencionarlos arriba, y amortizar el anticipo
--        también sobre indirectos + utilidad
-- ============================================================
-- Ajustes pedidos tras revisar la primera factura con indirectos y
-- utilidad (migración 084):
--
-- 1. Los 4 indirectos (IND-01..04) ya no se desglosan uno por uno --
--    se agrupan en una sola línea "Costos indirectos".
-- 2. Esa línea y la de "Utilidad del contratista" quedan al final del
--    desglose (ya iban al final por orden de inserción, sin cambios
--    ahí) y YA NO se mencionan en el texto de descripción de arriba de
--    la factura -- solo aparecen como línea en el desglose.
-- 3. La amortización de anticipo (lo que ya pagó el cliente por
--    adelantado) debe descontarse proporcionalmente de TODO lo que se
--    factura cada período -- actividades, indirectos Y utilidad -- no
--    solo de las actividades como hasta ahora. Para eso, el % de
--    anticipo ya no se calcula contra el presupuesto base (puro
--    costo) sino contra presupuesto_venta (el contrato completo,
--    costo + utilidad), y ese % se aplica sobre el bruto + utilidad
--    del período combinados en un solo monto de amortización.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- El % de anticipo ahora se mide contra el contrato completo
    -- (costo + utilidad), no solo contra el costo -- así el mismo %
    -- se puede aplicar de forma consistente sobre bruto + utilidad.
    ...
    -- Un solo monto de amortización, calculado sobre bruto (que ya
    -- incluye indirectos, migración 083) + utilidad combinados -- así
    -- el anticipo se descuenta proporcionalmente de los tres.
    ...
    -- Indirectos: agrupados en UNA sola línea al final (no desglosados
    -- por concepto). No tienen avance propio -- se facturan al mismo %
    -- de avance general del período que el resto del presupuesto.
    IF v_presupuesto_id IS NOT NULL THEN
      ...
      IF v_indirectos_total > 0 THEN
        v_desglose_act := v_desglose_act || jsonb_build_object(
          ...
          'monto_bruto', ROUND(v_indirectos_total * v_delta_pct / 100, 2)
        );
      END IF;
    END IF;

    -- Utilidad del contratista: también al final, también prorateada
    -- por el mismo avance general.
    ...
    -- La descripción de arriba ya NO menciona indirectos ni utilidad --
    -- esas ya se ven como línea propia en el desglose de abajo.
